...
class LocalGrouper(nn.Module):
    def __init__(self, channel, groups, kneighbors, use_xyz=True, normalize="anchor", **kwargs):
        ...
        self.knn = KNN(kneighbors,transpose_mode=True)
...
class PointMLP(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        self.tokenize = ConvBNReLU1D(1024,768,1)
    ...
    def forward(self, x, norm_plt, cls_label):
        ...
        # here is the decoder
        feat = self.tokenize(x)
        # print(x.shape) # B,D,N 2,1024,8
...
def pointMLP(num_classes=50, **kwargs) -> PointMLP:
    return PointMLP(num_classes=num_classes, points=1568, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[2, 2, 2, 1],
                 de_dims=[512, 256, 128, 128], de_blocks=[4,4,4,4],
                 gmp_dim=64,cls_dim=64, **kwargs)


if __name__ == '__main__':
    data = torch.rand(2, 3, 1568).to('cuda')
    norm = torch.rand(2, 4, 1568).to('cuda')
    cls_label = torch.rand([2, 16]).to('cuda')
    print("===> testing modelD ...")
    model = pointMLP(50).to('cuda')
    out,feat = model(data,norm, cls_label)  # [2,2048,50]
