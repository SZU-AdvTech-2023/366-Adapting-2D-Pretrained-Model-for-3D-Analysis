...
class SABlock(nn.Module):
    ...
    def forward(self, x, xyz):
        ...
        new_xyz, new_points, grouped_xyz, fps_idx = sample_and_group(npoint=N//self.stride,radius=self.radius,nsample=self.k,xyz=xyz,points=x)
...
class PointNextCls(nn.Module):
    def __init__(self, feat_dim = 1024,out_dim = 15):
        super().__init__()
...
if __name__ == '__main__':
    # PointNextEncoder()
    x = torch.randn(2,1568,4).to('cuda:3')
    xyz = x[:,:,:3]
    model = PointNextCls().to('cuda:3')
    print(model)
    out = model(x)
    print(out.shape)
