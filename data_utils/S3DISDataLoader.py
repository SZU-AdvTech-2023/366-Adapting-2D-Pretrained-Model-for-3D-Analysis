...
class S3DISDataset(Dataset):
    ...
    def __init__(self, split='train', data_root='trainval_fullarea', num_point=4096, test_area=5, block_size=1.0, sample_rate=1.0, transform=None):
        ...
        #train的话 rooms_split里面都是train的 rooms_split是一个个npy文件
        for room_name in tqdm(rooms_split, total=len(rooms_split)):
            ##room_name 代表一个npy
            room_path = os.path.join(data_root, room_name)
            #一个npy
            room_data = np.load(room_path)  # xyzrgbl, N*7 有标签的数据？
            points, labels = room_data[:, 0:6], room_data[:, 6]  # xyzrgb, N*6; l, N
            #hist,bin_edges 频数和边界 每次的频数都相加 ,是解包
            tmp, _ = np.histogram(labels, range(14))

            labelweights += tmp
            #points是二维数据，沿着第0轴计算？  x最小值，y最小值，z最小值
            coord_min, coord_max = np.amin(points, axis=0)[:3], np.amax(points, axis=0)[:3]
            #列表加入一个矩阵， 一个矩阵就是一个npy文件
            self.room_points.append(points), self.room_labels.append(labels)
            #该npy文件中最小的xyz 和最大的xyz  一维的
        ...
        #num_point_all 一个area的所有点 ，则sample_prob就是每一个area中的
        sample_prob = num_point_all / np.sum(num_point_all)
        #num_point_all应该是点的数量*采样率 / num_point  一次处理4096个点吗？
        #num_iter就是迭代次数 === 所有点的数量 * 采样率 / num_point  一次迭代4096个点？
        num_iter = int(np.sum(num_point_all) * sample_rate / num_point)
        room_idxs = []
        for index in range(len(rooms_split)):
            #生成重复元素的代码 index表示生成什么元素，append是添加一个元素，extend是迭代里面的东西
            #区域0的概率  * 迭代次数，就生成他的room_idxs
